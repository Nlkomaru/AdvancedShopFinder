package dev.nikomaru.advancedshopfinder.services
...
class ShopListPresenter : KoinComponent {
    ...
    suspend fun present(
        ...
        val (sellMsg, sellSum) = processShops(shops, sender, message, sum, options, ShopType.SELLING)
        message = sellMsg
        sum = sellSum

        val (buyMsg, buySum) = processShops(shops, sender, message, sum, options, ShopType.BUYING)
        message = buyMsg
        sum = buySum

        headerMessage?.let { sender.sendMessage(it) }
        sender.sendRichMessage("検索結果: ${sum}件")
    ...
    suspend fun processShops(
                ...
                .filter { shopItem ->
                    val hasStockOrSpace =
                        withContext(Dispatchers.minecraft) {
                            if (shopType == ShopType.SELLING) shopItem.remainingStock else shopItem.remainingSpace
                        } > 0

                    shopItem.shopType == shopType &&
                        (hasStockOrSpace || shopItem.isUnlimited || options.showNoStockShop)
                }.filter { shopItem ->
                    if (!options.showOwnerNotHasEnoughMoneyShop && !shopItem.isUnlimited && shopItem.isBuying) {
                        withContext(Dispatchers.minecraft) {
                            val ownerBalance = economy.getBalance(Bukkit.getOfflinePlayer(shopItem.owner.uniqueId!!))
                            val requiredAmount = shopItem.price * shopItem.shopStackingAmount
                            ownerBalance >= requiredAmount
                        }
                    } else {
                        true
                    }
        ...
        if (sender !is Player) {
            filteredShops =
                if (shopType == ShopType.SELLING) {
                    filteredShops.sortedByDescending { it.price }
                } else {
                    filteredShops.sortedBy { it.price }
                }
        } else {
            filteredShops =
                when (sortType) {
                    ...
                    SortType.DESC_DISTANCE_NEAREST -> filteredShops.sortedByDescending { getNearestPlaceDistance(it) }
                }
        }
        ...
        if (sender is Player) {
            ...
            val lightningCount = 10
            val lightningDistance = 200
            plugin.launch {
    ...
    private suspend fun sendShopInfo(
        sender: CommandSender,
        shopChest: Shop,
    ): Component {
        val nearPlace = getNearPlace(shopChest)
        val nearTownDistance =
            hypot(
                nearPlace!!.x.toDouble() - shopChest.location.blockX,
                nearPlace.z.toDouble() - shopChest.location.blockZ,
            )
    ...
    private fun getTags(
        ...
        return arrayOf(
            ...
            Placeholder.component("shop-type", mm.deserialize(if (shopChest.isBuying) "<color:red>買取" else "<color:green>販売")),
        )
    }

    private suspend fun getSellingShopCount(shopChest: Shop) =
        withContext(Dispatchers.minecraft) {
            if (shopChest.isUnlimited) {
                "無制限"
            } else {
    ...
    private fun getNearPlace(shopChest: Shop) =
        get<ConfigData>().placeData.minByOrNull {
